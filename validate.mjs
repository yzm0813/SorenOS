...
for (const feature of ['ChatView', 'WorkspaceView', 'MemoryView', 'TimelineView', 'SettingsView']) {
...
for (const route of ['/api/bootstrap', '/api/conversations', '/api/projects', '/api/memory/breath', '/api/mcp']) {
  if (!server.includes(route)) throw new Error(`Core route is missing: ${route}`);
}

console.log('Soren Phase 0 build contract validation passed.');
