...
export function createNotificationRoutes({db,events,push,http}:{db:any;events:any;push:any;http:any}):RouteHandler{return async({req,res,url,path})=>{
  ...
  if(req.method==='POST'&&path==='/api/push/subscribe'){try{http.json(res,201,{subscription:push.subscribe(await http.body(req))});}catch(error:any){http.json(res,/配置/.test(error?.message||'')?503:400,{message:error?.message||'订阅失败'});}return true;}
