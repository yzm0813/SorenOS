...
test('assembles identity, self state, memory and relevant contexts in stable order',()=>{const output=buildTurnContext({identity:'Stable Soren',selfState:'Reading systems papers',retrievedMemory:'Shared preference',recentMessages:[{id:'1',conversationId:'chat-b',role:'user',content:'Local question',quotedMessageId:null,createdAt:'2026-09-14T00:00:00Z'}],projectName:'Novel',socialContext:'用户评论了 Soren 的动态',runtimeState:'online',capabilities:'create reminder action',sceneInstruction:'work carefully'});for(const title of ['Soren Identity / Core','Soren Self State','Relevant Shared Memory','Project Context','Recent Conversation','Relevant Social Context','Current Runtime State','Available Capabilities','Scene Instruction'])assert.ok(output.includes(title));assert.ok(output.indexOf('Soren Self State')<output.indexOf('Relevant Shared Memory'));assert.ok(output.indexOf('Project Context')<output.indexOf('Recent Conversation'));assert.equal((output.match(/Stable Soren/g)||[]).length,1);});
