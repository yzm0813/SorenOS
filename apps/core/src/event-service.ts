...
export class EventService {
  ...
  private ensureProactiveConversation(preferred:string|null){if(preferred){const found=this.db.conversationById(preferred);if(found&&!found.archived)return found;}const saved=this.db.setting<string>('proactiveConversationId','');if(saved){const found=this.db.conversationById(saved);if(found&&!found.archived)return found;}const conversation=this.db.createConversation('Soren 主动消息');this.db.setSetting('proactiveConversationId',conversation.id);return conversation;}
