...
test('creates the current schema for a fresh database', () => {
  ...
  try {
    ...
    assert.ok(tables.includes('soren_self_state'));
    assert.ok(tables.includes('push_subscriptions'));
    assert.ok(tables.includes('push_deliveries'));
    store.db.close();
...
test('adopts the legacy schema without losing existing rows', () => {
  ...
  try {
    ...
    assert.deepEqual(migrated.db.prepare('SELECT version,name FROM schema_migrations ORDER BY version').all(), [{ version: 1, name: 'baseline_schema' },{version:2,name:'home_notes'},{version:3,name:'shared_memory_index'},{version:4,name:'moments_feed'},{version:5,name:'social_life_engine'},{version:6,name:'event_notification_layer'},{version:7,name:'cyberdaddy_supervision'},{version:8,name:'recurring_commitments'},{version:9,name:'soren_self_state'},{version:10,name:'web_push_delivery'}]);
    migrated.db.close();
...
test('upgrades Phase 3 Moments rows without changing their identity or timestamp',()=>{const{directory,file}=tempDatabase();try{const phase4=new Database(file);for(const migration of migrations.filter(item=>item.version<=4))phase4.exec(migration.sql);phase4.pragma('user_version = 4');phase4.prepare('INSERT INTO moments (id,author,content,location,world_context,read_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?)').run('kept-moment','soren','保留这条动态','','',null,'2026-09-15T12:00:00.000Z','2026-09-15T12:00:00.000Z');phase4.close();const migrated=new SorenDatabase(file),row=migrated.db.prepare('SELECT id,author_id,content,created_at FROM moments WHERE id=?').get('kept-moment') as any;assert.deepEqual(row,{id:'kept-moment',author_id:'soren',content:'保留这条动态',created_at:'2026-09-15T12:00:00.000Z'});assert.equal(migrated.db.pragma('user_version',{simple:true}),10);migrated.db.close();}finally{rmSync(directory,{recursive:true,force:true});}});
...
test('upgrades v8 to the independent Soren Self State schema',()=>{const{directory,file}=tempDatabase();try{const v8=new Database(file);for(const migration of migrations.filter(item=>item.version<=8))v8.exec(migration.sql);v8.pragma('user_version = 8');v8.close();const migrated=new SorenDatabase(file),table=migrated.db.prepare("SELECT name FROM sqlite_schema WHERE type='table' AND name='soren_self_state'").get() as any;assert.equal(table.name,'soren_self_state');assert.equal(migrated.db.pragma('user_version',{simple:true}),10);migrated.db.close();}finally{rmSync(directory,{recursive:true,force:true});}});

test('upgrades v9 to durable Web Push queues without changing notifications',()=>{const{directory,file}=tempDatabase();try{const v9=new Database(file);for(const migration of migrations.filter(item=>item.version<=9))v9.exec(migration.sql);v9.pragma('user_version = 9');const at='2026-09-17T10:00:00.000Z';v9.prepare('INSERT INTO domain_events (id,type,source_type,source_id,title,body,payload_json,dedupe_key,occurred_at) VALUES (?,?,?,?,?,?,?,?,?)').run('event-v9','assistant.proactive_message','soren','source','Soren','保留通知','{}','event-v9',at);v9.prepare('INSERT INTO notifications (id,event_id,type,source_type,source_id,delivery_channel,status,title,body,conversation_id,created_at,dedupe_key) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)').run('notification-v9','event-v9','assistant.proactive_message','soren','source','system','pending','Soren','保留通知',null,at,'event-v9');v9.close();const migrated=new SorenDatabase(file),row=migrated.db.prepare('SELECT id,body,message_id FROM notifications WHERE id=?').get('notification-v9') as any;assert.deepEqual(row,{id:'notification-v9',body:'保留通知',message_id:null});assert.equal(migrated.db.pragma('user_version',{simple:true}),10);migrated.db.close();}finally{rmSync(directory,{recursive:true,force:true});}});
