...
export class MomentsService {
  ...
  constructor(store:SorenDatabase){this.social=new SocialStore(store);this.social.ensureActors(socialActors);this.social.reconcileRecentUserPosts();this.social.reconcileRecentUserReplies();}
  ...
  publishUser(input:{content?:unknown;location?:unknown;worldContext?:unknown;imageDescription?:unknown}){const post=this.social.createPost('user',{content:clean(input.content,10000),location:clean(input.location,160),worldContext:clean(input.worldContext,500),imageDescription:clean(input.imageDescription,1000),motivation:'manual'}),dueAt=dueAfter(60_000,20*60_000);this.social.addEvent({type:'user_post',actorId:'soren',targetId:post.id,summary:`用户发布了一条朋友圈：${post.content.slice(0,500)}`,importance:8,dueAt,metadata:{momentId:post.id}});this.social.nudgeEvaluation(dueAt);return post;}
